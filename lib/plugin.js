...
const buildCssModulesJs = async ({ fullPath, options, build }) => {
  ...
  const cssModulesJSON = exports;
  
  const classNamesMapString = JSON.stringify(Object.fromEntries(Object.entries(cssModulesJSON).map(([name, val]) => [kebabCase(name), val])), undefined, 2);
